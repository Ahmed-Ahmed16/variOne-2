...
private:
    ...
    bool ensureAttestationCert(const uint8_t attPriv[32]) {
        ...
        do {
            ...
            if (rc != 0) break;
            rc = mbedtls_x509write_crt_set_subject_name(&crt, "CN=VariOne U2F Attestation,O=VariOneDevices,C=US");
            if (rc != 0) break;
            rc = mbedtls_x509write_crt_set_issuer_name(&crt, "CN=VariOne U2F Attestation,O=VariOneDevices,C=US");
            if (rc != 0) break;
