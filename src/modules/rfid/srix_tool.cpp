...
void SRIXTool::save_file() {
    ...
    if (!(*fs).exists("/VariRFID/SRIX")) (*fs).mkdir("/VariRFID/SRIX");

    // Check if the file already exists and add number
    String filepath = "/VariRFID/SRIX/" + filename;
...
void SRIXTool::load_file() {
    ...
    if (!(*fs).exists("/VariRFID/SRIX")) {
        displayError("No dumps found!");
        delay(1500);
        displayError("Folder /VariRFID/SRIX missing");
        delay(2000);
        set_state(IDLE_MODE);
        return;
    }
    ...
    File dir = (*fs).open("/VariRFID/SRIX");
    ...
    for (const String &fname : fileList) {
        options.emplace_back(fname, [this, fname, fs]() { load_file_data(fs, "/VariRFID/SRIX/" + fname); });
    }
...
void SRIXTool::load_file_data(FS *fs, String filepath) {
    ...
    int lastSlash = filename.lastIndexOf('/');
    if (lastSlash >= 0) {
        filename = filename.substring(lastSlash + 1); // Remove "/VariRFID/SRIX/"
    }
