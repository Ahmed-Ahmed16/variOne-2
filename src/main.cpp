...
void setup() {
    ...
    // Never persist WiFi config to NVS. The attack code reconfigures the AP/STA
    // many times and a write corrupted during a deauth TX power-sag poisons NVS,
    // making SoftAP report success but never broadcast (survives reboot, only a
    // full chip erase clears it). Disabling persistence removes the writes
    // entirely, so the WiFi NVS can never rot again.
