...
uint16_t Adafruit_PN532::getLastPassiveTargetAtqa(void) { return _lastAtqa; }

uint8_t Adafruit_PN532::getLastPassiveTargetSak(void) { return _lastSak; }
...
/**************************************************************************/
/*!
    @brief   VariOne: emulate ISO14443A 106kbps card with caller-specified
             UID and SAK. Builds TgInitAsTarget (0x8C) packet and submits it
             to the PN532. Returns 1 when an external reader has activated
             the emulation, 0 on timeout / no reader.

    @param   uid3  Pointer to first 3 bytes of NFCID1 (PN532 derives BCC).
    @param   sak   SEL_RES byte (0x08 = Mifare Classic 1K, 0x20 = ISO14443-4).
*/
/**************************************************************************/
uint8_t Adafruit_PN532::AsTargetUID(const uint8_t *uid3, uint8_t sak) {
  uint8_t target[] = {
      0x8C,                    // TgInitAsTarget
      0x00,                    // Mode: passive only, 106A
      0x04, 0x00,              // SENS_RES (ATQA Mifare Classic 1K)
      uid3[0], uid3[1], uid3[2], // NFCID1[3] — caller supplied
      sak,                     // SEL_RES (SAK)
      // FelicaParams (18 zeros — Felica not used)
  ...
  // read response packet — 0x15 in the activation field signals an
  // initiator (reader) has selected us.
  readdata(pn532_packetbuffer, 8);
  return (pn532_packetbuffer[6] == 0x15) ? 1 : 0;
}
