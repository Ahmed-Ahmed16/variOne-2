...
public:
  ...
  uint16_t getLastPassiveTargetAtqa(void);
  uint8_t getLastPassiveTargetSak(void);
  bool inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response,
                      uint8_t *responseLength);
  bool inListPassiveTarget();
  uint8_t AsTarget();
  // VariOne: emulate ISO14443A card with caller-supplied UID + SAK.
  // uid3 = first 3 bytes of UID (PN532 derives BCC from these).
  // sak  = SEL_RES (e.g. 0x08 Mifare Classic 1K, 0x20 ISO14443-4).
  uint8_t AsTargetUID(const uint8_t *uid3, uint8_t sak);
